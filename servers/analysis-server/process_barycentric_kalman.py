    ...
    for r in edges[t]:
      try:
        ...
        while i< len(cache['anchors']):
          # pdb.set_trace()
          if cache['anchors'][i]['device']['location'] != 'null' or cache['anchors'][i]['device'] != 'null':
...
def read_file(fileName):
  ...
  with open(fileName, 'r') as f:
    next(f)
    reader = csv.reader(f)
    rssiDataList = sorted(list(reader), key=operator.itemgetter(2), reverse=False)
...
def main():
  ...
  while i<len(rssiDataList):
    ref = int(float(rssiDataList[i][2]))
    # pdb.set_trace()
    ...
    currentData = {"transmitterId": beaconDev, "receiverId": rssiDataList[i][0], "mu": rssiDataList[i][1]}
...
# t1.setDaemon(True)
# t1.start()
read_file('./' + sys.argv[1] + 'actlab_rssi_Test3_06Oct_kalmanFiltered1-2.csv')
