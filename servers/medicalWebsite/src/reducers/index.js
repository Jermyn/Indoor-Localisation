...
const rootReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case FETCH_DEVICES:
            return {
                ...state, devices: action.payload, staticDevices: action.payload.filter(function ({type}) {
                    return type === 'static';
                })
            };
        ...
        case FETCH_DEVICE_LOCATIONS:
            return {
                ...state,
                deviceLogs: action.payload.aggregations.id.buckets.map((x) => x.latest.hits.hits[0]._source)
            };
