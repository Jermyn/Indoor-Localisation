...
import AnchorsInfoTable from './anchorInfoTable'
...
class AnchorInfo extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (!_.isEqual(prevProps.rooms, this.props.rooms)) {
        ...
        var assignedAnchors = this.assignedAnchors(this.props.rooms, this.props.staticDevices)
        this.setState({roomData})
        this.setState({assignedAnchors})
    }
  ...
  assignedAnchors = (rooms) => {
    let data = []
    let isAssigned = null
    rooms.some(room => {
        isAssigned = room.anchors ? room.anchors : false
        if (isAssigned) return true;
    })
    let item = isAssigned ? isAssigned : void 0
    if (item != undefined) {
      ...
      <div className={classes.root}>
        ...
        <form className={classes.align} onSubmit={this.handleSubmit}>
          <Grid container
            ...
            >
              <Grid item xs={12}>
                <AnchorsInfoTable rooms={roomData} />
              </Grid>
            </Grid>
...
const ConnectedAnchorInfo = connect(mapStateToProps, mapDispatchToProps)(AnchorInfo);

export default withStyles(styles, {withTheme: true})(ConnectedAnchorInfo);
