...
from bokeh.models.widgets import PreText,Select, Button, Panel, Tabs
...
dataHomeList.append("accX,accY,accZ,magX,magY,magZ,gyroX,gyroY,gyroZ,timeStamp,sysTime,id,Count")
dataHrList.append("hr,timeStamp,sysTime")
dataCounterList.append("Count")
...
def endProcess():
    global initialTime, endTime, flag, currentMode, button, ecgFile, homeFile, hrFile, dataHomeList, dataEcgList, dataHrList, dataCounterList
    global done
    ...
    write_file =str("01_Counter" + str(int(time.time())) + ".csv")
    with open(write_file, "w") as output:
        for line in dataCounterList:
            output.write(line + '\n')
    dataHrList=[]
    print("Saved to Counter file")
    ...
    dataHomeList.append("accX,accY,accZ,magX,magY,magZ,gyroX,gyroY,gyroZ,timeStamp,sysTime, id,counter")
...
def ecgSensor(x):
    ...
    if flag==0:
        if initialTime ==0:
            ...
            dataEcgList.append(str(ecg1Sample1) + "," + str(ecg1Sample2) + "," + str(ecg2Sample1) + "," + str(ecg2Sample2) + "," + str(ResRate1Sample1) + "," + str(ResRate1Sample2) + "," + str(ResRate2Sample1) + "," + str(ResRate2Sample2) + "," + str(ct) + "," + str(int(time.time()*1000)) + "," + str(currentTimeStamp) + "," + str(x['uuid']) + "," + str(sequenceNumber))
...
#     x = requests.recv_json()
    ...
    if flag==0:
        if initialTime == 0:
            ...
            dataHomeList.append(str(accX) + "," + str(accY) + "," + str(accZ) + "," + str(magX) + "," + str(magY) + "," + str(magZ) + "," + str(gyroX) + "," + str(gyroY) + "," + str(gyroZ) + "," + str(timestamp) + "," + str(int(time.time()*1000)) + "," + str(x['uuid']) + "," + str(counter))
...
def main():
    global flag, done, ctHR, currentCharac, devices, deviceQueue, currentUUID, sensor, sourceAx, sourceAy, sourceAz, sourceAx2, sourceAy2, sourceAz2 
    # print(len(dataQueue))
    # print (deviceQueue)
...
def dataReceive():
    global dataQueue, deviceQueue
    global currentCharac, currentUUID, currentMode, dataEcgList, dataHrList, dataHomeList, endTime, timeQueue, button, sensor, counter, dataCounterList
    ...
    while True:
        try:
            ...
            if x['uuid'] == currentUUID and x['characteristic']==currentCharac:
                ...
                counter += 1
                dataCounterList.append(str(counter))
            elif currentMode!=0:
                dataQueue.append(x)
                counter += 1
                dataCounterList.append(str(counter))
...
curdoc().add_root(column(row(ticker1, ticker2, duration, button), tabs))
