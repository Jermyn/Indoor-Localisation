...
from aoa import angleOfArrival
...
cacheUpdate  = zmq.Context().socket(zmq.SUB)
cacheUpdate.setsockopt_string(zmq.SUBSCRIBE, config['notifications']['cacheUpdate'])
cacheUpdate.connect(config['zmqSockets']['broker']['xpub'])
cacheReq     = zmq.Context().socket(zmq.REQ)
cacheReq.connect(config['zmqSockets']['serverRequests']['reqrep'])
...
def updateCache():
  global cache
  cacheReq.send_string(config['notifications']['cacheRequest'])
  cache = cacheReq.recv_json()
...
def processEdges():
  ...
  # angle of arrival
  locations     = angleOfArrival(locations, transmitters)
  ...
  for k, (mapId, pos_observation) in locations.items():
    ...
    if (
      ...
      and (
        ...
        or distance(history[k]['pos_update'], pos_posterior, scale) > 2
      )
    ):
      history[k].update({
...
def listenForCacheUpdates():
  while True:
    [topic, message] = cacheUpdate.recv_multipart()
