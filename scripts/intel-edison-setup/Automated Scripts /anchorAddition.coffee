...
attemptDiscoverServices = (peripheral, uuids) ->
  if peripheral.state == 'connected'
    console.log "discoverovering services #{uuids}"
...
readPoxData = (data) ->
  ...
  if newData.length >= 19
    if newData[0] == 170 && newData[1] == 85 && newData[2] == 15 && newData[3] == 8 && newData[4] == 1 && newData[5] > 0 && newData[6] > 0
      date = new Date()
      spO2 = newData[5]
      bpm = newData[6]
      time = date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + ":" + date.getMilliseconds()
    if (newData[11] == 170 && newData[12] == 85 && newData[13] == 15 && newData[14] == 8 && newData[15] == 1 && newData[16] > 0 && newData[17] > 0)
      date = new Date()
      spO2 = newData[16]
      bpm = newData[17]
      time = date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + ":" + date.getMilliseconds()
...
processData = (data, characteristics) ->
  if characteristics.toString() == '6e400003b5a3f393e0a9e50e24dcca9e'
...
gattAdditionRequestObservable = (peripheral) ->
  if peripheral.advertisement.localName.indexOf("RHYTHM") !=-1
    ...
    type = "ecg"
  else if peripheral.advertisement.localName.indexOf("PC-60F") != -1
  	type = "pox"
  return Rx.Observable.bindNodeCallback(requester.request.bind(requester))(
    [
...
gattBeaconsToAdd$ = _macBeacons$
                    .filter (peripheral) -> 
                      (!cache.gatts?[peripheral.uuid] && peripheral.state == 'disconnected') && (peripheral.advertisement.localName.indexOf("RHYTHM") !=-1 || peripheral.advertisement.localName.indexOf("HomeRehab") !=-1 || peripheral.advertisement.localName.indexOf("BELBLE") !=-1 || peripheral.advertisement.localName.indexOf("PC-60F") !=-1)
                    .share()

gattBeacons$      = _macBeacons$
                    .filter (peripheral) -> 
                      peripheral.state == 'disconnected' && peripheral.advertisement.localName=='iChoice'
                      # cache.gatts?[peripheral.uuid] && cache.gatts?[peripheral.uuid]?.connect && peripheral.state == 'disconnected'
...
.subscribe ([peripheralUuid, serviceUuid, characteristicsDesc]) ->
  ...
  characteristicsDesc.forEach ({uuid}) ->
    if cache.gatts[peripheralUuid].profile[serviceUuid][uuid]
      ...
      # write
      # console.log (cache.gatts[peripheralUuid].profile[serviceUuid][uuid].write)
...
.subscribe ([peripheralUuid, serviceUuid, characteristicUuid, data, isNotification]) ->
  [spO2, bpm] = processData(data, characteristicUuid)
  if spO2 > 0 && bpm > 0
    _vitals$.next({gattid: cache.gatts?[peripheralUuid]?.device?.id, heart_rate:bpm, spo2:spO2, anchorId: env.ANCHOR})
  # sendAchorData({
  #   uuid:           peripheralUuid
  #   service:        serviceUuid
