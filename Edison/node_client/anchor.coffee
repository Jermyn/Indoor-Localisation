...
gattConnections$  = gattBeacons$
                    ...
                    .flatMap (peripheral) ->
                      ...
                      resetLoss(peripheral.uuid)
                      return gattRequestObservable(peripheral)
                      .filter ({connect}) -> 
                        connect && peripheral.state == 'disconnected'
                      .map ({lease, minRssi, maxLoss}) ->
                        return { peripheral, lease, minRssi , maxLoss}
                    .retry()
...
# gatt
gattConnections$
.subscribe ({peripheral, lease, delay, minRssi, maxLoss}) ->
  peripheral.contract = { lease, minRssi, delay , maxLoss}
  if delay?
...
.subscribe (uuid) ->
  ...
  # disconnect when data efficiency is bad
  if peripheral.contract?.maxLoss?
    console.log "connect until consecutive data loss > #{peripheral.contract.maxLoss}"
    Rx.Observable.interval(4)
    .takeUntil(nobleDisconnect$.filter (uuid) -> uuid == peripheral.uuid)
    .subscribe ->
      try
        if loss[uuid].cl > peripheral.contract.maxLoss   
          console.log "consecutive data loss exceeded for #{peripheral.uuid}"
          attemptDisconnectPeripheral(peripheral)
      catch err
        return null
